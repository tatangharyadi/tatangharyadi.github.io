...
| --- | --- | --- |
| F06-AC01 | ONNX Runtime Web can run the `vae_encoder` graph from `schmuell/sd-turbo-ort-web` against an uploaded photo and produce a latent the UNet loop can partially noise and continue from. | **Verified — see Pending.** A local prototype ran the full `text_encoder → vae_encoder → unet → vae_decoder` pipeline against a real uploaded photo, at two `strength` values, and produced a photo-derived stylized output. `js/muse.js` itself is still unwritten; this confirms the mechanism, not the shipped implementation. |
| F06-AC02 | The stylization model's actual fetched size (not the 2.58GB unquantized repo total) is measured and judged acceptable for a one-time page load. | **Resolved — see Pending.** The prototype's live fetch from `schmuell/sd-turbo-ort-web` totals 2.58GB across the four components it uses, matching the repo listing exactly (not an overestimate); accepted as the size budget for this spec. `js/muse.js`'s own network-panel transfer still needs confirming at implementation time, but no smaller viable mirror of this exact repo exists. |
| F06-AC03 | End-to-end generation (detect face → stylize → 5 poses) completes in a tolerable time on ordinary consumer hardware, not just a high-end discrete GPU. | **Partially measured — see Pending.** One end-to-end img2img pass measured ~2–2.5s on a real MacBook Pro (Apple M4 Pro), all WebGPU, across two runs — but that is a single pose, a premium (not "ordinary") device, and excludes face detection and session-load time. The 5-pose sequence and a lower-end device are both still untested. |
